...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray feature = jsonRootObject.getJSONArray("features");

            for (int i=0 ; i<feature.length(); i++){
                JSONObject currentEarthquake = feature.getJSONObject(i);
                JSONObject properties = currentEarthquake.getJSONObject("properties");
                double mag = properties.getDouble("mag");
                String location = properties.getString("place");
                String timeStr = properties.getString("time");
                String magStr = String.valueOf(mag);

                Date dateObject = new Date(Long.parseLong(timeStr));
                SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM DD, yyyy");
                String date = dateFormatter.format(dateObject);

                SimpleDateFormat dateFormatter2 = new SimpleDateFormat("h:mm a");
                String time = dateFormatter2.format(dateObject);

                earthquakes.add(new Earthquake(magStr,location,date,time));
